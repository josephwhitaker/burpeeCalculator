...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kBurpeeCalculatorUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kBurpeeCalculatorUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}

...
//Rare exceptions to indicate a bug in the _burpeecalculator code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_burpeecalculator = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_burpeecalculator]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.burpeecalculator.module"

...
NSString * BurpeeCalculator$ModuleRequireFormat = @"(function(exports){"
...
@implementation BurpeeCalculatorObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_burpeecalculator);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create BurpeeCalculator global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "BurpeeCalculator" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_burpeecalculator = [[BurpeeCalculatorObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_burpeecalculator];
	
	NSString *_burpeecalculatorNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _burpeecalculatorNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_burpeecalculator valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:BurpeeCalculator$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.burpeecalculator.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.burpeecalculator.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
